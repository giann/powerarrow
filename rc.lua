...
local config_dir = ("/home/rom/.config/awesome/")
...
local modkey        = "Mod4"
local terminal      = "terminal --geometry=164x50+101+60"
local terminalr     = "sudo terminal --default-working-directory=/root/ --geometry=200x49+80+36"
local configuration = "TERM=screen-256color lilyterm -T 'Awesome Configuration' -g 228x62+0+16 -x ~/.gem/ruby/1.9.1/bin/mux start configuration"
local rttmux        = "sudo terminal --geometry=220x59+20+36 --default-working-directory=/root/ -x tmux -2"
local ttmux         = "lilyterm -T tmux -g 221x60+20+36 -e tmux -2"
local tetmux        = "terminal --geometry=189x54+20+36 -x tmux -2"
local sakura        = "sakura -c 222 -r 60 --geometry=+15+30"
local lilyterm      = "lilyterm -g 221x60+20+36"
local musicplr      = "lilyterm -T Music -g 130x34-320+16 -e ncmpcpp"
local iptraf        = "lilyterm -T 'IP traffic monitor' -g 180x54-20+34 -e sudo iptraf-ng -i all"
local mailmutt      = "lilyterm -T 'Mutt' -g 140x44-20+34 -e mutt"
local chat          = "TERM=screen-256color lilyterm -T 'Chat' -g 228x62+0+16 -x ~/.gem/ruby/1.9.1/bin/mux start chat"
...
local mylauncher = awful.widget.launcher({ image = image(beautiful.clear_icon), menu = mymainmenu })
...
for s = 1, screen.count() do
    ...
    local chaticon = widget ({type = "imagebox" })
    ...
    local music = widget ({type = "imagebox" })
    ...
    local task_warrior = blingbling.task_warrior.new(beautiful.widget_task)
    ...
    local sensors = widget({ type = "textbox" })
    ...
    local udisks_glue = blingbling.udisks_glue.new(beautiful.widget_hdd)
    ...
    local neticon = widget ({type = "imagebox" })
    ...
    binaryclock.timeout = binaryclock.show_sec and 1 or 20
    ...
    binaryclock.paintdot = function(val, shift, limit) 
        ...
        for i = 0, limit - 1 do
            ...
            binaryclock.widget.image:draw_rectangle(
                shift,
                binaryclock.h - binaryclock.dotsize - height,
                binaryclock.dotsize,
                binaryclock.dotsize,
                true,
                string.sub(binval,limit-i,limit-i) == "1" and binaryclock.color_active or binaryclock.color_inactive
            )
    ...
    binaryclock.drawclock = function ()
        ...
        local col_count = binaryclock.show_sec and 6 or 4
    ...
    local binarytimer = timer { timeout = binaryclock.timeout }
    ...
    local spr = widget({ type = "textbox" })
    spr.text = ' '
    local sprd = widget({ type = "textbox" })
    sprd.text = '<span background="#313131" font="Terminus 12"> </span>'
    local spr3f = widget({ type = "textbox" })
    spr3f.text = '<span background="#777e76" font="Terminus 12"> </span>'
    local arr1 = widget ({type = "imagebox" })
    arr1.image = image(beautiful.arr1)
    local arr2 = widget ({type = "imagebox" })
    arr2.image = image(beautiful.arr2)
    local arr3 = widget ({type = "imagebox" })
    arr3.image = image(beautiful.arr3)
    local arr4 = widget ({type = "imagebox" })
    arr4.image = image(beautiful.arr4)
    local arr5 = widget ({type = "imagebox" })
    arr5.image = image(beautiful.arr5)
    local arr6 = widget ({type = "imagebox" })
    arr6.image = image(beautiful.arr6)
    local arr7 = widget ({type = "imagebox" })
    arr7.image = image(beautiful.arr7)
    local arr8 = widget ({type = "imagebox" })
    arr8.image = image(beautiful.arr8)
    local arr9 = widget ({type = "imagebox" })
    arr9.image = image(beautiful.arr9)
    local arr0 = widget ({type = "imagebox" })
